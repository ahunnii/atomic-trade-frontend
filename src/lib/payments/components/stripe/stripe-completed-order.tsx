        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              Order {order?.orderNumber ?? session.id}
            </h1>
            <p className="mt-1 text-sm text-gray-500">
              Confirmed {new Date(session.created * 1000).toLocaleDateString()}
            </p>
          </div>
        </div>
        ...
        <div className="h-fit space-y-4 rounded-lg border bg-white p-5">
          ...
          <div className="space-y-2 border-t pt-4">
            <div className="flex justify-between text-sm">
              <span>Subtotal</span>
              <span>{formatCurrency(session.amount_subtotal ?? 0)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>Shipping</span>
              <span>
                {formatCurrency(session.total_details?.amount_shipping ?? 0)}
              </span>
            </div>
            <div className="flex justify-between text-base font-bold">
              <span>Total</span>
              <span>{formatCurrency(session.amount_total ?? 0)}</span>
            </div>
          </div>
        </div>
      </div>
